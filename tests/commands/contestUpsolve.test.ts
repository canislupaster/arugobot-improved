    ...
    options: {
      ...
      getBoolean: jest.fn((name: string) => {
        if (name === "force_refresh") {
          return options.forceRefresh ?? null;
        }
        return null;
      }),
...
describe("contestUpsolveCommand", () => {
  ...
  it("forces a contest solves refresh when requested", async () => {
    const interaction = createInteraction({ query: "1234", forceRefresh: true });
    const getContestSolvesResult = jest.fn().mockResolvedValue({
      solves: [],
      source: "api",
      isStale: false,
    });
    ...
    expect(getContestSolvesResult).toHaveBeenCalledWith(1234, 0);
  });
});
